# Bopacorp CRM — Mobile & Backend Integration System Design
...
To secure mobile sessions and prevent unauthorized token extraction, the application uses a dual-token strategy:
...
5.  If rotation fails (e.g. token expired/revoked), the session is terminated, and the user is redirected to the login screen.
...
*   **Form & API Validation**: Mobile forms and API requests are validated locally using the same Zod schemas (e.g. `CreateBusinessClientRequestSchema`, `CreateNegotiationRequestSchema`) that the backend uses to validate incoming requests.
...
## 4. API Endpoint Map for Mobile Client

The mobile app connects to the following routes:

### 4.1 Public Endpoints
*   `POST /api/v1/auth/login` — Authenticates credentials and returns a token pair.
*   `POST /api/v1/auth/refresh` — Rotates expired access tokens using the refresh token.
*   `POST /api/v1/employability/apply` — Accepts application form data along with `multipart/form-data` PDF resume uploads.

### 4.2 Authenticated Endpoints
*   `GET /api/v1/crm/business-clients` — Lists assigned corporate clients.
*   `POST /api/v1/crm/business-clients` — Creates a new client profile.
*   `GET /api/v1/crm/negotiations` — Lists negotiations/deals.
...
Since mobile advisors operate in the field where network coverage is volatile:
